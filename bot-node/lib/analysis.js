...
// /markets is GET (no body), and returns the market config object — but for
// briefings we want the INDEX summary (IHSG level + % change), so hit /market-summary instead.
const getMarket = (m) =>
  fetch(`${BACKEND}/market-summary?market=${encodeURIComponent(m)}`)
